...
if __name__ == "__main__":
    ...
    hr_play = {"play_id": "P555", "description": "HR to RF"}
    print("home_run #1:", deduper.is_new_alert(game_id, "home_run", hr_play))  # True
    print("home_run #2 (dup):", deduper.is_new_alert(game_id, "home_run", hr_play))  # False

    # Cleanup demo
    deduper.cleanup_old_alerts()
    print("cleanup ran OK")


# ========================
# File: tests/test_dedup.py
# ========================
# Pytest suite for AlertDeduper. Creates a controllable monotonic clock via monkeypatch.
...
def clock(monkeypatch):
    c = Clock()
    # monkeypatch time.monotonic used inside dedup
    import time as _time
    monkeypatch.setattr(_time, "monotonic", c.now, raising=False)
    return c
...
def _bases_loaded_state(game_id="G1", inning=1, top=True, outs=0, batter_id="B1", ab=1):
    return {
        "game_id": game_id,
        "inning": inning,
        "inning_top": top,
...
def test_plate_appearance_scope_allows_next_batter(deduper, clock):
    d = deduper
    s1 = _bases_loaded_state(batter_id="B1", ab=10)
    s2 = _bases_loaded_state(batter_id="B2", ab=11)

    assert d.is_new_alert("G1", "bases_loaded_no_outs", s1) is True
    # same batter immediately should be blocked
    assert d.is_new_alert("G1", "bases_loaded_no_outs", s1) is False
    # next batter should pass
    assert d.is_new_alert("G1", "bases_loaded_no_outs", s2) is True


def test_play_scope_is_strictly_once(deduper, clock):
    d = deduper
    play = {"play_id": "P123", "description": "HR to LF"}
    assert d.is_new_alert("G2", "home_run", play) is True
    assert d.is_new_alert("G2", "home_run", play) is False


def test_time_window_throttle(deduper, clock):
    d = deduper
    state = _bases_loaded_state(batter_id="B9", ab=99)
    assert d.is_new_alert("G3", "bases_loaded_no_outs", state) is True
    # within 60s window should be blocked
    assert d.is_new_alert("G3", "bases_loaded_no_outs", state) is False
    # advance just past window
    clock.advance(61)
    # state unchanged, but re-alert_after=180, so still blocked
    assert d.is_new_alert("G3", "bases_loaded_no_outs", state) is False


def test_realert_after_allows_after_interval(deduper, clock):
    d = deduper
    state = _bases_loaded_state(batter_id="B5", ab=50)
    assert d.is_new_alert("G4", "bases_loaded_no_outs", state) is True
    # advance past 60s window but before 180s realert
    clock.advance(65)
    assert d.is_new_alert("G4", "bases_loaded_no_outs", state) is False
    # advance beyond realert interval
    clock.advance(180)
    assert d.is_new_alert("G4", "bases_loaded_no_outs", state) is True


def test_cleanup_prunes_old_entries(deduper, clock):
    d = deduper
    state = _bases_loaded_state(batter_id="B7", ab=70)
    assert d.is_new_alert("G5", "bases_loaded_no_outs", state) is True
    # move far beyond window+buffer and retention
    clock.advance(1000)
    # should not raise; and internal dicts should shrink
    before_ra = len(d.recent_alerts)
    before_ls = len(d.last_sent_by_simple)
    d.cleanup_old_alerts()
    assert len(d.recent_alerts) <= before_ra
    assert len(d.last_sent_by_simple) <= before_ls


def test_token_bucket_limits_bursts(monkeypatch):
    # separate instance with tight bucket
    d = AlertDeduper(enable_buckets=True, bucket_capacity=2, bucket_refill_seconds=10)
    game = "G6"
    s = _bases_loaded_state(game_id=game)
    # first two pass
    assert d.is_new_alert(game, "bases_loaded_no_outs", s) is True
    s["batter_id"] = "B2"; s["at_bat_index"] = 2
    assert d.is_new_alert(game, "bases_loaded_no_outs", s) is True
    # third should be dropped by bucket
    s["batter_id"] = "B3"; s["at_bat_index"] = 3
    assert d.is_new_alert(game, "bases_loaded_no_outs", s) is False
...
# ========================
# Minimal test deps for Replit runner
pytest>=8.0.0
...
# File: .replit
# ========================
# Replit run configuration: press ▶️ to run tests
run = "python -m pip install -r requirements.txt && python run_tests.py"
